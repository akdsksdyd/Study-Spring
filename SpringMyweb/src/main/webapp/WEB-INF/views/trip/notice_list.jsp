...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
	...
	<div id="container">
		...
		<div class="bodytext_area box_inner">
			...
			<%-- 
			컨트롤러에서 사용자 클래스를 매개변수로 지정하면 화면으로 넘어오는 이름이
			클래스명이 소문자로 지정되서 자동으로 다음화면으로 전달된다.
			${criteria } === @ModelAttribute("criteria") 해 준 것과 같다.
			--%>
			
			<!-- 5.맨 처음 -->
			<div class="pagination">
				<a href="notice_list?page=1&amount=${pageVO.amount }&searchType=${criteria.searchType}&searchName=${criteria.searchName}" class="firstpage  pbtn">
					<img src="${pageContext.request.contextPath }/resources/img/btn_firstpage.png" alt="첫 페이지로 이동">
				</a>
				
				<!-- 3.이전 페이지네이션 -->
				<c:if test="${pageVO.prev }">
					<a href="notice_list?page=${pageVO.start-1 }&amount=${pageVO.amount }&searchType=${criteria.searchType}&searchName=${criteria.searchName}" class="prevpage  pbtn">
				...
				<c:forEach var="num" begin="${pageVO.start }" end="${pageVO.end }">
					<a href="notice_list?page=${num }&amount=${pageVO.amount }&searchType=${criteria.searchType}&searchName=${criteria.searchName}">
						<span class="pagenum ${pageVO.page == num ? 'currentpage' : ''}">${num }</span>
					</a>
				</c:forEach>
				
				<!-- 2.다음 페이지네이션 -->
				<c:if test="${pageVO.next }">
					<a href="notice_list?page=${pageVO.end+1 }&amount=${pageVO.amount }&searchType=${criteria.searchType}&searchName=${criteria.searchName}" class="nextpage  pbtn">
						<img src="${pageContext.request.contextPath }/resources/img/btn_nextpage.png" alt="다음 페이지로 이동">
					</a>
				</c:if>
				
				<!-- 4.맨 마지막 -->
				<a href="notice_list?page=${pageVO.realEnd }&amount=${pageVO.amount }&searchType=${criteria.searchType}&searchName=${criteria.searchName}" class="lastpage  pbtn">
